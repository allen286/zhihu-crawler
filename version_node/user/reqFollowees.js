...
const USER_TABLE = 'zhihudata.hostids'
const FOLLOW_TABLE = 'zhihudata.follow'
const SELECT_SQL = `SELECT DISTINCT hostId FROM ${USER_TABLE};`
const INSERT_SQL = `INSERT INTO ${FOLLOW_TABLE} SET ?`
const TIME_OUT = 0
let seqIndex = 0
let idList = []
...
function cb(res) {
  ...
  setTimeout(() => {
    ...
    options.path = `/people/${id}/followees?limit=20&offset=0`
...
function save2db(actCount, id) {
  // 插入到数据库中
  try {
    conn.insert(INSERT_SQL, {actCount, id}, () => console.log(`第${++seqIndex}条数据插入成功，id: ${id}, count: ${actCount}`))
  } catch (error) {
